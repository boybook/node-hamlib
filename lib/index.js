...
class MemoryFacade {
  ...
  async layout() {
    return this._nativeInstance.getMemoryLayout();
  }

  async capabilities(channelNumber) {
    if (channelNumber !== undefined) {
      return this._nativeInstance.getMemoryCapabilities(channelNumber);
    }
    return this._nativeInstance.getMemoryCapabilities();
  }
  ...
  async get(channelNumber, options = {}) {
  ...
  async set(channel) {
    if (!channel || typeof channel.channelNumber !== 'number') {
      throw new TypeError('memory.set() requires channel.channelNumber');
    }
    return this._nativeInstance.setMemoryChannel(channel.channelNumber, channel);
  }

  async list(options = {}) {
    return this._nativeInstance.getMemoryList(options);
  }

  async setMany(channels, options = {}) {
    if (!Array.isArray(channels)) {
      throw new TypeError('memory.setMany() requires an array');
    }
    return this._nativeInstance.setMemoryChannels(channels, options);
  }

  async replaceAll(channels) {
    if (!Array.isArray(channels)) {
      throw new TypeError('memory.replaceAll() requires an array');
    }
    return this._nativeInstance.replaceMemoryChannels(channels);
  }
  ...
  async select(channelNumber, vfo) {
    if (vfo !== undefined) {
      return this._nativeInstance.selectMemoryChannel(channelNumber, vfo);
    }
...
class HamLib extends EventEmitter {
  ...
  constructor(model, port) {
    ...
    this.memory = new MemoryFacade(this._nativeInstance);
  }
   ...
   * Enabled by default; set NODE_HAMLIB_GLOBAL_LOCK=0 to start disabled.
   * Async calls wait up to NODE_HAMLIB_GLOBAL_LOCK_TIMEOUT_MS (default 5000).
   * @param {boolean} enabled
   * @static
   */
  static setGlobalLockEnabled(enabled) {
    return nativeModule.HamLib.setGlobalLockEnabled(enabled);
  }

  /**
   * Check whether process-wide HamLib radio call serialization is enabled.
   * @returns {boolean}
   * @static
   */
  static isGlobalLockEnabled() {
    return nativeModule.HamLib.isGlobalLockEnabled();
  }
   ...
   * @param {number} model - Radio model number
   * @returns {Array<Object>} List of config field descriptors
   * @static
   */
  static getConfigSchemaForModel(model) {
    return nativeModule.HamLib.getConfigSchemaForModel(model);
  }
   ...
   * @static
   */
  static getPortCapsForModel(model) {
    return nativeModule.HamLib.getPortCapsForModel(model);
  }
  ...
  async setMemoryChannel(channelNumber, channelData) {
    return this.memory.set({ ...channelData, channelNumber });
  }
  ...
  async getMemoryChannel(channelNumber, readOnly) {
    if (typeof readOnly !== 'boolean') {
      return this._nativeInstance.getMemoryChannel(channelNumber);
    }
    return this.memory.get(channelNumber, { readOnly });
  }
   ...
   */
  async selectMemoryChannel(channelNumber, vfo) {
    return this.memory.select(channelNumber, vfo);
  }
   ...
   */
  async getMem(vfo) {
    return this.memory.current(vfo);
  }
   ...
   */
  async setBank(bank, vfo) {
    return this.memory.setBank(bank, vfo);
  }
   ...
   */
  async sendRawWrite(data) {
    await this._nativeInstance.sendRaw(data, 0);
  }
   ...
   */
  async getAttenuatorValues() {
    return this._nativeInstance.getAttenuatorValues();
  }

  /**
   * Get supported AGC mode values
   * @returns {number[]} Array of Hamlib AGC enum values
   */
  async getAgcLevels() {
    return this._nativeInstance.getAgcLevels();
  }
   ...
   * @param {string} levelType - Level type ('RFPOWER', 'AF', 'SQL', etc.)
   * @returns {{ min: number, max: number, step: number, kind: 'float' | 'int', source: string } | null}
   */
  async getLevelGranularity(levelType) {
    return this._nativeInstance.getLevelGranularity(levelType);
  }

  /**
   * Get a discrete RF power step table for the current rig, frequency, and mode.
   * Returns null when Hamlib does not expose reliable step metadata.
   * @param {number} frequency - Frequency in Hz
   * @param {string} mode - Radio mode ('USB', 'LSB', 'FM', etc.)
   * @returns {Array<{ normalized: number, milliwatts: number, watts: number, rigUnitRange?: { current: number, min: number, max: number } }> | null}
   */
  async getRfPowerStepTable(frequency, mode) {
    return this._nativeInstance.getRfPowerStepTable(frequency, mode);
  }
